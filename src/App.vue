...
<template>
  <div class="bg-gray-100 min-h-screen flex items-center justify-center px-4 flex-col gap-10">
    <div class="bg-white shadow-lg rounded-2xl p-8 w-full max-w-4xl flex flex-col items-center">
      ...
      <form class="flex items-center gap-2 mb-6 max-w-md" @submit.prevent="onSubmit">
        <input id="ipInput" type="text" placeholder="Enter IP address"
          class="flex-1 px- 4 py-2 rounded-lg border border-gray-300 focus:outline-none focus:ring-2 focus:ring-blue-500 w-md p-2"
          v-model="ipInput" />
    ...
    <IpInfoBento :data="data" v-if="data && !('error' in data)" />

    <ErrorCard v-if="data && 'error' in data && data.error" :reason="data.reason" :message="data.message" />
  </div>
</template>
<style>
@import 'tailwindcss';
